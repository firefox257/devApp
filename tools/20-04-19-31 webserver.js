...
async function handleApiRequest(req, res, relativePath) {
    ...
    if (apiCache.has(relativePath)) {
        ...
        try {
            ...
            console.error(`Error executing cached API ${relativePath}:`, error);
            sendPlainTextResponse(res, '500 Internal Server Error', 500);
        }
    } else {
        fs.access(apiFilePath, fs.constants.F_OK, async (err) => {
            ...
            try {
                ...
                if (typeof apiModule.handler === 'function') {
                    ...
                    sendPlainTextResponse(res, `500 API Error: ${relativePath} does not export a 'handler' function.`, 500);
                }
            } catch (error) {
                console.error(`Error loading or executing API ${relativePath}:`, error);
                sendPlainTextResponse(res, '500 Internal Server Error', 500);
            }
        });
    }
        ...
        fs.access(wsFilePath, fs.constants.F_OK, (err) => {
            ...
            if (wsCache.has(cacheKey)) {
                ...
                try {
                    ...
                    console.error(`[WS] ${serverType} Load error ${cacheKey}:`, e.message);
            ...
            socket.on('data', (chunk) => {
                ...
                while (frameBuffer.length >= 2) {
                    ...
                    try {
                        ...
                        console.error(`[WS] ${serverType} Handler error ${cacheKey}:`, e.message);
            ...
            try {
                ...
                console.error(`[WS] ${serverType} Init error ${cacheKey}:`, e.message);

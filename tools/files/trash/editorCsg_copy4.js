...
function loadPagesIntoEditor(editorElement, pages) {
	...
	// Clear existing contexts (remove all except we'll rebuild)
	const existingContexts = instance.listContexts()
	existingContexts.forEach(ctx => {
		instance.removeContext(ctx.id)
	})
	
	// Load pages as contexts
	if (pages && pages.length > 0) {
		pages.forEach((page, index) => {
			...
			if (index === 0) {
				// Add first context and switch to it
				instance.addContext(contextName, lines, { switchTo: true })
			} else {
				// Add additional contexts
				instance.addContext(contextName, lines, { switchTo: false })
			}
		})
	}
}

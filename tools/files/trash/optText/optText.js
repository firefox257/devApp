...
let lastInputValue = ''; // ✅ iOS FIX: Track previous value to handle autocorrect quirks
...
function ensureSharedHiddenInput() {
	...
	// ✅ iOS FIX: Removed pointer-events:none, kept transparent colors and 16px font
	...
	// --- Handle Text Input (IME / Typing / iOS Quirks) ---
	sharedHiddenInput.addEventListener('input', (e) => {
		...
		// 1. Handle Deletions (Backspace/Delete)
		...
		// 3. Handle Text Insertion
			...
			if (e.data) {
				...
				// ✅ iOS FIX: Autocorrect/suggestion quirk. e.data is null, text is in value.
		...
		// Reset for next keystroke
		lastInputValue = currentValue;
		sharedHiddenInput.value = '';
		lastInputValue = ''; 
	});

	// --- Handle ALL Keydown (Shortcuts + Navigation) ---
	sharedHiddenInput.addEventListener('keydown', (e) => {
		...
		// ✅ Pass the event directly to the instance's key handler
		activeInstance._handleKey(e);
	});

	// --- Handle Blur (Smart Focus Tracking) ---
	sharedHiddenInput.addEventListener('blur', () => {
		setTimeout(() => {
			if (!activeInstance) return;
			const ae = document.activeElement;
			
			// ✅ iOS FIX: If the shared input itself is still active, do nothing
			if (ae === sharedHiddenInput) return;
			
			// Allow modals to keep focus
			if (ae === activeInstance._gotoInput || activeInstance._modalOverlay.contains(ae)) {
				return;
			}
			
			// If focus moved within the editor (e.g. toolbar button), steal it back
			if (activeInstance.contains(ae)) {
				sharedHiddenInput.focus();
			} else {
				// Truly left the editor
				activeInstance = null;
			}
		}, 0);
	});
...
function createOptTextInstance(originalElement = null, initialData = null) {
	...
	let touch = { lastY: 0, lastX: 0, lastTime: 0, velocityY: 0, velocityX: 0, isScrolling: false, startTime: 0, startY: 0, startX: 0, momentumId: null, lastMomentumTime: 0, touchedHandle: null, didScroll: false, scrollYAtGrab: 0, scrollXAtGrab: 0, isDown:false, isTouchSequence: false };
			...
			if (def.attribute && def.property) {
				...
				if (attrValue) {
					container[def.property] = (instance) => {
						try {
							if (typeof window[attrValue] === 'function') { window[attrValue](instance); return; }
							const fn = new Function('instance', 'event', attrValue);
							const result = fn(instance, instance);
							if (typeof result === 'function') result(instance);
						} catch (err) { console.error(`[optText] Error executing ${def.attribute} attribute:`, err); }
					};
				}
			}
			if (def.stateConfig) {
	...
	if (typeof additionManager !== 'undefined') {
		if (typeof additionManager.injectToolbarButtons === 'function') additionManager.injectToolbarButtons(container, dataManager, container.extensions);
		if (typeof additionManager.injectDropdownItems === 'function') additionManager.injectDropdownItems(container, dataManager, container.extensions);
		if (typeof additionManager.injectAutoInitAdditions === 'function') additionManager.injectAutoInitAdditions(container, dataManager, container.extensions);
		for (const def of additionManager.registry.values()) {
			if (def.stateConfig) {
				for (const [stateName, config] of Object.entries(def.stateConfig)) {
	...
	function _createDiffCommand(before, after, reason) {
		...
		let startLine = 0;
		while (startLine < bLines.length && startLine < aLines.length && bLines[startLine] === aLines[startLine]) { startLine++; }
		let endLineB = bLines.length - 1;
		let endLineA = aLines.length - 1;
		while (endLineB >= startLine && endLineA >= startLine && bLines[endLineB] === aLines[endLineA]) { endLineB--; endLineA--; }
		...
		if (startLine > delEndLine) { delEndLine = startLine; delEndCol = 0; }
	...
	function _updateSelectButtonState() {
		const btn = container.querySelector('[data-action="toggle-select"]');
		if (!btn) return;
		if (selection.active) {
			...
			btn.setAttribute('aria-label', 'Start Select');
			btn.title = 'Start Select';
		}
	}

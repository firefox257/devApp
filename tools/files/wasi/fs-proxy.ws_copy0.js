...
// 🔧 FIX: Add promisified versions of writeFile/readFile (were callback-based, causing ERR_INVALID_ARG_TYPE)
...
async function cleanupVirtualSymlink(realPath) {
  ...
  } catch (e) { /* ignore - meta may not exist */ }
}
      ...
      try {
        ...
        switch (cmd) {
          ...
          case 'open-at': {
            ...
            let osFd;
            try { osFd = await fsOpen(targetPath, nodeFlags); }
          ...
          // 🔧 FIX: symlink case - accept oldPath field, validate target, use promisified fsWriteFile
          case 'symlink': {
            // 🔍 DEBUG: Log exact message
            console.log(`[WASI2-FS] 🔍 SYMLINK MSG:`, JSON.stringify({
              id: msg.id, cmd: msg.cmd, 
              path: msg.path, oldPath: msg.oldPath, target: msg.target,
              newpath: msg.newpath, newPath: msg.newPath, link: msg.link,
              newDirFd: msg.newDirFd, dirFd: msg.dirFd
            }, null, 2));
            ...
            console.debug(`[WASI2-FS] symlink: target="${target}", linkName="${linkName}", dirFd=${dirFd}`);
            ...
            try {
              ...
              // iOS: Defensive placeholder (skip if restricted)
              try {
                const exists = await fsStat(linkLocation).catch(() => null);
                if (!exists) {
                  // 🔧 FIX: Use promisified fsWriteFile (not callback-based fs.writeFile)
              ...
              // 🔧 FIX: Write ACTUAL target — NO fallback to '.'

...
class TopsParser {
  ...
  parseStatement() {
    ...
    if (lowerT === 'iswitch') {
      ...
      for (const { value, body } of caseBlocks) {
        genCode.push(
          ...
          0x04, 0x40,
          ...body,
          0x0c, 0x01,  // br 1 to exit outer block (switch)
          0x0b
        );
      }

      // ✅ FIX: Wrap default in always-true if block for consistent branch depth
      if (hasDefault) {
        genCode.push(
          0x41, ...this.sleb(1),  // i32.const 1 (always true)
          0x04, 0x40,             // if void
          ...defaultCode,
          0x0c, 0x01,             // br 1 to exit switch block
          0x0b                    // end if
        );
      }

      genCode.push(0x0b); // end block
      return [...switchCode, ...genCode];
    }

    // --- FSWITCH IMPLEMENTATION ---
    if (lowerT === 'fswitch') {
    ...
    if (lowerT === 'break') {
      ...
      if (this.peek() === ';') this.consume(';');
      let breakLevel = 0;
      // ✅ FIX: If inside a switch, break out of the outer block (level 1)
      if (this.switchDepthStack.length > 0 && this.switchDepthStack[this.switchDepthStack.length - 1] === this.controlDepth - 1) {
        breakLevel = 1;
        this.switchDepthStack.pop();

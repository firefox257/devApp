...
export function newOptText(target = null, initialData = null) {
	// Allow passing an options object as the first argument (e.g., newOptText({ contexts: [...] }))
	...
	// Create the core editor instance
	const instance = createOptTextInstance(target, initialData);

	// If no target element was provided, wrap it in a floating dialog (modal)
	if (!target) {
...
// 1. Expose ONLY newOptText to globalThis for standard, non-module access
if (typeof globalThis !== 'undefined') {
	globalThis.newOptText = newOptText;
}

// 2. Handle auto-initialization for <opttext> tags
if (typeof document !== 'undefined') {
	const processOptTextTags = () => {
		document.querySelectorAll('opttext').forEach(el => {
			...
			if (p) {
				// Use the unified newOptText function to replace the tag
	...
	// A. Handle initial synchronous load
	...
	// B. Handle dynamic additions (e.g., element.innerHTML = "<opttext></opttext>")
	const observer = new MutationObserver((mutations) => {
		for (const mutation of mutations) {
			if (mutation.addedNodes.length > 0) {
				for (const node of mutation.addedNodes) {
					if (node.nodeType === 1) { // Element node
						// Check if the node itself is <opttext> or contains one
	...
	// Start observing the document body for newly added <opttext> elements

...
additionManager.register({
	...
	// ✅ FIX: Cleaned up the malformed template literal from the provided code
	toolUI: (api) => `<input type="text" id="search-input" placeholder="Find text..." style="font-size:12px;height:20px;padding:0 6px;border:1px solid var(--ot-border,#ccc);border-radius:4px;outline:none;width:100px;flex-shrink:0;background:var(--ot-bg-input,#fff);color:var(--ot-text,#000);box-sizing:border-box;"> <button class="opt-text-toolbar-btn" data-action="prev" title="Previous match">↑</button> <button class="opt-text-toolbar-btn" data-action="next" title="Next match">↓</button> <span id="search-status" style="font-size:11px;color:var(--ot-text-muted,#666);min-width:40px;text-align:center;margin-left:auto;flex-shrink:0;">0/0</span>`,
	...
	init: (api, container, dataManager) => {
		...
		// ✅ FIX: Universal auto-init now runs `init` for ALL additions. 
		// Since searchTool's UI is only injected on activation, we must skip 
		// initialization if the elements don't exist yet to prevent null reference errors.
		// It will be called again properly when the tool is actually activated.

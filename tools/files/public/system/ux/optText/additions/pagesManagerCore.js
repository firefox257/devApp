// ./system/ux/optText/additions/pagesManagerCore.js
...
export function updatePagesToolbar() {
...
export function init(api, container, dataManager) {
    ...
    // ✅ FIX: Use native Object.defineProperty directly to avoid lifecycle timing crashes
    if (!container.hasOwnProperty('_pagesPropsDefined')) {
        Object.defineProperty(container, 'pages', {
            get() {
            ...
            set(newValues) {
                ...
                if (this._onpagechange) {
                    this._onpagechange.call(this, { pageIndex: 0, title: newValues[0].title, content: newValues[0].content });
                }
            },
            configurable: true
        });

        Object.defineProperty(container, 'pageIndex', {
            get() {
                const ctxs = this.listContexts();
            ...
            set(newIndex) {
                ...
                if (newIndex >= 0 && newIndex < ctxs.length) {
                    ...
                    updatePagesToolbar();
                    if (this._onpagechange) {
                        this._onpagechange.call(this, { pageIndex: newIndex, title: ctxs[newIndex].name, content: this.value });
                    }
                }
            },
            configurable: true
    ...
    // ✅ Early return during auto-init (tool UI doesn't exist yet)
    const tool = container.querySelector('#opt-addition-active-tool');
    if (!tool) return;
    ...
    // ✅ Guard against duplicate global listeners if init() is called multiple times
    if (!pagesState.changeHandler) {
    ...
    if (titleInput) {
        titleInput.addEventListener('input', (e) => {
            ...
            if (container.dataManager) {
                if (container.dataManager.current) container.dataManager.current.name = newName;
                const ctx = container.dataManager.contexts.find(c => c.id === container.contextId);
                if (ctx) ctx.name = newName;
            }
        });

        const finalizeRename = () => updatePagesToolbar();
    ...
    if (!pagesState.dropdownMenu) {
        ...
        pagesState.dropdownMenu.style.cssText = `
            ...
            background: var(--ot-bg-canvas, #fff); border: 1px solid var(--ot-border, #ccc);
    ...
    const renderDropdown = () => {
        ...
        pagesState.dropdownMenu.innerHTML = ctxs.map(ctx => {
            ...
            return `<div class="opt-text-dropdown-item" data-id="${ctx.id}" style="padding:8px 14px; font-size:12px; cursor:pointer; background:${bg}; color:var(--ot-text-primary,#000); white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">
                ${isActive ? '<span style="font-weight:bold;">✓ </span>' : '<span style="opacity:0;">✓ </span>'}${ctx.name}
             </div>`;
        }).join('');
        ...
        pagesState.dropdownMenu.querySelectorAll('.opt-text-dropdown-item').forEach(item => {
            item.addEventListener('click', (e) => {
                ...
                if (selectedCtx && titleInput) titleInput.value = selectedCtx.name;
                pagesState.dropdownMenu.style.display = 'none';
            });
            item.addEventListener('mouseenter', () => {
                if (item.getAttribute('data-id') !== container.contextId) item.style.background = 'var(--ot-bg-hover, #f0f0f0)';
            });
            item.addEventListener('mouseleave', () => {
                if (item.getAttribute('data-id') !== container.contextId) item.style.background = 'transparent';
            });
        });
    };
    ...
    // ✅ Guard against duplicate document listeners
    ...
    if (!pagesState.deleteModal) {
        ...
        pagesState.deleteModal.querySelector('[data-action="confirm"]').addEventListener('click', () => {
            ...
            container.switchContext(ctxs[targetIdx].id);
            updatePagesToolbar();
            api.ui.toast('Page deleted');
        });
        pagesState.deleteModal.addEventListener('click', (e) => {
            if (e.target === pagesState.deleteModal) hideModal();
        });
    }

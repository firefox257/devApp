...
function setupFilePickerInstance(originalElement = null) {
    ...
    // --- UPDATE BUTTON ICONS WITH MATERIAL ICONS (self-hosted font) ---
    createButton.innerHTML = '<i class="material-icon material-icon-lg icon-create" aria-hidden="true">create_new_folder</i>';
    createButton.title = "Create New File or Directory";

    renameButton.innerHTML = '<i class="material-icon material-icon-lg icon-rename" aria-hidden="true">edit</i>';
    renameButton.title = "Rename";

    copyButton.innerHTML = '<i class="material-icon material-icon-lg icon-copy" aria-hidden="true">content_copy</i>';
    copyButton.title = "Copy Selected";

    cutButton.innerHTML = '<i class="material-icon material-icon-lg icon-cut" aria-hidden="true">content_cut</i>';
    cutButton.title = "Cut Selected";

    pasteButton.innerHTML = '<i class="material-icon material-icon-lg icon-paste" aria-hidden="true">content_paste</i>';
    pasteButton.title = "Paste";

    deleteButton.innerHTML = '<i class="material-icon material-icon-lg icon-delete" aria-hidden="true">delete_outline</i>';
    deleteButton.title = "Delete Selected";

    downloadButton.innerHTML = '<i class="material-icon material-icon-lg icon-download" aria-hidden="true">file_download</i>';
    downloadButton.title = 'Download/Upload';

    cancelButton.innerHTML = '<i class="material-icon material-icon-lg icon-cancel" aria-hidden="true">close</i>';
    cancelButton.title = "Cancel";

    usePathButton.innerHTML = '<i class="material-icon material-icon-lg icon-confirm" aria-hidden="true">check</i>';
    usePathButton.title = "Use Selected File Path";

    refreshButton.innerHTML = '<i class="material-icon material-icon-sm icon-refresh" aria-hidden="true">refresh</i>';
    ...
    const updateButtonStates = () => {
        ...
        if (isSingleFileSelection) {
            ...
            downloadButton.innerHTML = '<i class="material-icon material-icon-lg icon-upload" aria-hidden="true">file_upload</i>';
        ...
        try {
            ...
            if (instanceCurrentPath !== '/') {
                ...
                fileListHtml += `
                    <tr class="up-directory-row" data-type="directory">
                        <td><i class="material-icon material-icon-sm icon-up-dir" aria-hidden="true">arrow_upward</i></td>
            ...
            files.forEach(file => {
                const icon = file.type === 'directory' 
                    ? '<i class="material-icon material-icon-sm icon-folder" aria-hidden="true">folder</i>' 
                    : '<i class="material-icon material-icon-sm icon-file" aria-hidden="true">insert_drive_file</i>';
    ...
    // ✅ FIXED: Properly handles selection state for both files AND directories
    // 🔧 FIX v2.7.1: Removed buggy directory fallback that was re-adding 'selected' class
    const updateSelectionState = (path) => {
        const selectedCheckboxes = fileListTbody.querySelectorAll('.file-checkbox:checked');
        const checkedCount = selectedCheckboxes.length;
        
        // ✅ STEP 1: ALWAYS remove selected class from ALL rows first
        ...
        if (checkedCount === 1) {
            ...
            // ✅ Add selected class ONLY to the checked row
            ...
            // ✅ Add selected class to ALL checked rows
            ...
            // ✅ NO CHECKBOXES CHECKED: Clear all selection state
            // 🔧 FIX: Removed the buggy directory fallback that was re-adding selected class
            instanceSelectedFilePath = null;
            instanceSelectedFileType = null;
            titleTextEl.textContent = 'No file selected';
            // ✅ No rows should have the selected class when nothing is checked
        }
    ...
    // ✅ UPDATED: Adds selected class when checking/unchecking
    selectAllCheckbox.addEventListener('change', (e) => {
        const isChecked = e.target.checked;
        const checkboxes = fileListTbody.querySelectorAll('.file-checkbox');
        
        // ✅ Clear existing selected classes first
        fileListTbody.querySelectorAll('tr').forEach(row => {
            row.classList.remove('selected', 'file-picker-selected-row');
        });
        ...
        checkboxes.forEach(cb => {
            if (!cb.closest('.up-directory-row')) {
                ...
                // ✅ Add selected class when checking
                if (isChecked) {
                    const row = cb.closest('tr');
                    if (row) row.classList.add('selected', 'file-picker-selected-row');
                }

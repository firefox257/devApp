        ...
        <div class="file-picker-creation-dialog">
            ...
            <div class="file-picker-creation-dialog-options">
                <label>
                    ...
                    <span class="material-icon material-icon-sm icon-file" aria-hidden="true">insert_drive_file</span> File
                </label>
                <label>
                    <input type="radio" name="creationType" value="directory">
                    <span class="material-icon material-icon-sm icon-folder" aria-hidden="true">folder</span> Directory
                </label>
            </div>
...
export const BUTTON_ICONS = {
    create: { icon: 'create_new_folder', title: "Create New File or Directory", class: 'icon-create' },
    rename: { icon: 'edit', title: "Rename", class: 'icon-rename' },
    copy: { icon: 'content_copy', title: "Copy Selected", class: 'icon-copy' },
    cut: { icon: 'content_cut', title: "Cut Selected", class: 'icon-cut' },
    paste: { icon: 'content_paste', title: "Paste", class: 'icon-paste' },
    delete: { icon: 'delete_outline', title: "Delete Selected", class: 'icon-delete' },
    download: { icon: 'file_download', title: 'Download/Upload', class: 'icon-download' },
    upload: { icon: 'file_upload', title: 'Upload File to Current Directory', class: 'icon-upload' },
    cancel: { icon: 'close', title: "Cancel", class: 'icon-cancel' },
    confirm: { icon: 'check', title: "Use Selected File Path", class: 'icon-confirm' },
    refresh: { icon: 'refresh', title: "Refresh", class: 'icon-refresh', size: 'sm' },
    upDir: { icon: 'arrow_upward', title: "Parent directory", class: 'icon-up-dir', size: 'sm' },
    folder: { icon: 'folder', class: 'icon-folder', size: 'sm' },
    file: { icon: 'insert_drive_file', class: 'icon-file', size: 'sm' }
};
...
export function injectStyles() {
    ...
    /* ===== COLUMN LAYOUT — ICON + CHECKBOX + NAME LEFT-ALIGNED (NO SPACE) ===== */
    
    /* Column 1: Icon - narrow, right-aligned content */
    .file-picker-list-table td:nth-child(1),
    .file-picker-list-table th:nth-child(1) {
    ...
    /* Column 2: Checkbox - narrow, left-aligned */
    ...
    /* Column 3: Name - left-aligned, minimal padding */
    ...
    /* Column 4: Size - right-aligned, with left border for separation */
    .file-picker-list-table td:nth-child(4),
    .file-picker-list-table th:nth-child(4) {
        width: 80px !important;
        padding: 4px 8px !important;
        text-align: right !important;
        border-left: 1px solid var(--system-border) !important;
    }
    ...
    .file-picker-popup {
        ...
        z-index: var(--z-index-global-popup, 9999);
        opacity: 0;
        visibility: hidden;
...
// --- NEW: Instance-scoped popup message ---
export function showInstancePopupMessage(pickerContainer, message, isError = false) {
...
// --- NEW: Apply icons to toolbar buttons ---
export function applyButtonIcons(pickerContainer, iconSet = BUTTON_ICONS) {
    const buttonMap = [
        { selector: '.create-btn', key: 'create', sizeClass: 'lg' },
        { selector: '.rename-btn', key: 'rename', sizeClass: 'lg' },
        { selector: '.copy-btn', key: 'copy', sizeClass: 'lg' },
        { selector: '.cut-btn', key: 'cut', sizeClass: 'lg' },
        { selector: '.paste-btn', key: 'paste', sizeClass: 'lg' },
        { selector: '.delete-btn', key: 'delete', sizeClass: 'lg' },
        { selector: '.cancel-btn', key: 'cancel', sizeClass: 'lg' },
        { selector: '.use-path-btn', key: 'confirm', sizeClass: 'lg' },
        { selector: '.file-picker-refresh-button', key: 'refresh', sizeClass: 'sm' }
    ];

    buttonMap.forEach(({ selector, key, sizeClass }) => {
        const btn = pickerContainer.querySelector(selector);
        const iconDef = iconSet[key];
        if (btn && iconDef) {
            const size = sizeClass === 'sm' ? 'material-icon-sm' : 'material-icon-lg';
            btn.innerHTML = `<i class="material-icon ${size} ${iconDef.class}" aria-hidden="true">${iconDef.icon}</i>`;
            if (iconDef.title) btn.title = iconDef.title;
        }
    });
}

// --- NEW: Set download/upload button icon dynamically ---
export function setDownloadButtonIcon(downloadButton, isFileDownload) {
    const iconDef = isFileDownload ? BUTTON_ICONS.download : BUTTON_ICONS.upload;
    const sizeClass = 'material-icon-lg';
    downloadButton.innerHTML = `<i class="material-icon ${sizeClass} ${iconDef.class}" aria-hidden="true">${iconDef.icon}</i>`;
    downloadButton.title = iconDef.title;
}

// --- NEW: Add download/upload button to toolbar ---
export function addDownloadButtonToToolbar(pickerContainer) {
    const downloadButton = document.createElement('button');
    downloadButton.className = 'download-btn system-btn system-btn-icon';
    ...
    if (deleteCell && menuRow) {
        ...
        if (cancelButtonCell) {
            ...
            menuRow.appendChild(downloadCell);
        }
    }
...
// --- NEW: Generate file list row HTML ---
export function generateFileRowHTML(file, normalizedCurrentPath) {
    const iconDef = file.type === 'directory' ? BUTTON_ICONS.folder : BUTTON_ICONS.file;
    const icon = `<i class="material-icon material-icon-sm ${iconDef.class}" aria-hidden="true">${iconDef.icon}</i>`;
    ...
    const nameClass = file.type === 'directory' ? 'file-name directory-name' : 'file-name';

    return `
        <tr ${dataType}>
            <td class="file-icon">${icon}</td>
            <td><input type="checkbox" class="file-checkbox" data-path="${fullPath}" aria-label="Select ${file.name}"></td>
            <td class="${nameClass}" data-path="${fullPath}">${file.name}</td>
            <td class="file-size-cell">${size}</td>
        </tr>
    `;
}

// --- NEW: Generate "up directory" row HTML ---
export function generateUpDirectoryRowHTML(upPath) {
    const iconDef = BUTTON_ICONS.upDir;
    return `
        <tr class="up-directory-row" data-type="directory">
            <td><i class="material-icon material-icon-sm ${iconDef.class}" aria-hidden="true">${iconDef.icon}</i></td>
...
// --- NEW: Create picker DOM (exported for consistency) ---

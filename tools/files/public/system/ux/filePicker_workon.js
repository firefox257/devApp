...
import { 
    ...
    generateFileRowHTML,
    generateUpDirectoryRowHTML,
    showInstancePopupMessage
...
function setupFilePickerInstance(originalElement = null) {
    ...
    // --- DOWNLOAD/UPLOAD BUTTON ADDITION (delegated to UI module) ---
    const downloadButton = addDownloadButtonToToolbar(pickerContainer);
    ...
    // --- UPDATE BUTTON ICONS WITH MATERIAL ICONS (delegated to UI module) ---
    applyButtonIcons(pickerContainer, BUTTON_ICONS);
    // Set initial download button state
    setDownloadButtonIcon(downloadButton, false); // default to upload icon
    ...
    // --- INSTANCE-SPECIFIC DIALOG FUNCTIONS (using UI templates) ---
    
    const showConfirmDialog = (message) => {
        return new Promise(resolve => {
            instanceDialogOverlay.innerHTML = DIALOG_TEMPLATES.confirm(message);
    ...
    const showPromptDialog = (message, defaultValue = '') => {
        return new Promise(resolve => {
            instanceDialogOverlay.innerHTML = DIALOG_TEMPLATES.prompt(message, defaultValue);
    ...
    const showCreationDialog = () => {
        return new Promise(resolve => {
            instanceDialogOverlay.innerHTML = DIALOG_TEMPLATES.creation();
    ...
    const updateButtonStates = () => {
        ...
        // Update download button icon based on selection
        setDownloadButtonIcon(downloadButton, isSingleFileSelection);
    ...
    const renderFileList = async (path) => {
        ...
        try {
            ...
            if (instanceCurrentPath !== '/') {
                ...
                fileListHtml += generateUpDirectoryRowHTML(upPath);
            }
            ...
            files.forEach(file => {
                fileListHtml += generateFileRowHTML(file, normalizedCurrentPath);
            });
    ...
    const updateSelectionState = (path) => {
        ...
        fileListTbody.querySelectorAll('tr').forEach(row => {
        ...
        if (checkedCount === 1) {
            ...
            instanceSelectedFileType = row?.dataset.type || null;
            titleTextEl.textContent = instanceSelectedFilePath;
            
            if (row) row.classList.add('selected', 'file-picker-selected-row');
    ...
    const handleRename = async () => {
        ...
        if (selected.length !== 1) {
            showInstancePopupMessage(pickerContainer, "Please select exactly one item to rename.", true);
            return;
        }
        ...
        if (newName === null || !newName.trim()) {
            showInstancePopupMessage(pickerContainer, "Rename cancelled.", true);
            return;
        }
        ...
        try {
            ...
            showInstancePopupMessage(pickerContainer, `Renamed '${oldName}' to '${newName}' successfully.`);
    ...
    const handleCreate = async () => {
        ...
        if (result === null || !result.name.trim()) {
            showInstancePopupMessage(pickerContainer, "Creation cancelled.", true);
            return;
        }
        ...
        try {
            if (type === 'file') {
                ...
                showInstancePopupMessage(pickerContainer, `File '${normalizedName}' created successfully.`);
            } else {
                await api.mkPath(fullPath);
                showInstancePopupMessage(pickerContainer, `Directory '${normalizedName}' created successfully.`);
            }
            renderFileList(instanceCurrentPath);
        } catch (error) {
    ...
    const handleCopy = () => {
        ...
        if (selected.length === 0) {
            showInstancePopupMessage(pickerContainer, "No items selected to copy.", true);
            return;
        }
        instanceClipboard.type = 'copy';
        instanceClipboard.paths = selected;
        showInstancePopupMessage(pickerContainer, `Copied ${selected.length} item(s) to clipboard.`);
        updateButtonStates();
    };
    ...
    const handleCut = () => {
        const selected = getSelectedPaths();
        if (selected.length === 0) {
            showInstancePopupMessage(pickerContainer, "No items selected to cut.", true);
            return;
        }
        instanceClipboard.type = 'cut';
        instanceClipboard.paths = selected;
        showInstancePopupMessage(pickerContainer, `Cut ${selected.length} item(s) to clipboard.`);
        updateButtonStates();
    };

    const handlePaste = async () => {
        if (instanceClipboard.type === null || instanceClipboard.paths.length === 0) {
            showInstancePopupMessage(pickerContainer, "Clipboard is empty.", true);
            return;
        }
        ...
        for (const sourcePath of instanceClipboard.paths) {
            ...
            try {
                ...
                showInstancePopupMessage(pickerContainer, `Failed to ${instanceClipboard.type} ${sourcePath.split('/').pop()}: ${error.message}`, true);
                failCount++;
            }
        }

        if (successCount > 0) {
            showInstancePopupMessage(pickerContainer, `${successCount} item(s) ${instanceClipboard.type}ed successfully.`);
        }
        if (failCount > 0) {
            showInstancePopupMessage(pickerContainer, `${failCount} item(s) failed to ${instanceClipboard.type}.`, true);
        }
    ...
    const handleDelete = async () => {
        const selected = getSelectedPaths();
        if (selected.length === 0) {
            showInstancePopupMessage(pickerContainer, "No items selected to delete.", true);
            return;
        }
        ...
        if (!confirm) {
            showInstancePopupMessage(pickerContainer, "Deletion cancelled.", true);
            return;
        }
        ...
        for (const name of selected) {
            try {
                ...
                showInstancePopupMessage(pickerContainer, `Failed to delete/move ${name}: ${error.message}`, true);
                failCount++;
            }
        }

        if (successCount > 0) {
            showInstancePopupMessage(pickerContainer, `${successCount} item(s) ${instanceCurrentPath.startsWith('/trash/') ? 'deleted permanently' : 'moved to trash'} successfully.`);
        }
        if (failCount > 0) {
            showInstancePopupMessage(pickerContainer, `${failCount} item(s) failed to delete/move.`, true);
        }
    ...
    const handleDownload = async () => {
        showInstancePopupMessage(pickerContainer, `Starting download for ${instanceSelectedFilePath.split('/').pop()}...`);
        ...
        try {
            ...
            showInstancePopupMessage(pickerContainer, `Download of '${fileName}' completed.`);
    ...
    const handleUpload = () => {
        ...
        fileInput.addEventListener('change', async (e) => {
            ...
            showInstancePopupMessage(pickerContainer, `Uploading '${file.name}' (${formatBytes(file.size)})...`);
            ...
            try {
                ...
                xhr.upload.addEventListener('progress', (event) => {
                    if (event.lengthComputable) {
                        ...
                        showInstancePopupMessage(pickerContainer, `Uploading '${file.name}': ${percentComplete}%`);
                    }
                });
                
                xhr.addEventListener('load', () => {
                    if (xhr.status === 200) {
                        showInstancePopupMessage(pickerContainer, `File '${file.name}' uploaded successfully!`);
                        renderFileList(instanceCurrentPath);
                    } else {
                ...
                xhr.addEventListener('abort', () => {
                    showInstancePopupMessage(pickerContainer, `Upload cancelled`, true);
                });
    ...
    const handleUsePath = (e) => {
        if (!instanceSelectedFilePath || instanceSelectedFilePath === 'Multiple files selected') {
            showInstancePopupMessage(pickerContainer, "Please select a single file or directory path to use.", true);
            return;
        }
    ...
    fileListTbody.addEventListener('click', (e) => {
        ...
        if (nameCell && !isCheckboxClick) {
            if (isDirectory) {
                if (e.target.classList.contains('file-name') || e.target.classList.contains('directory-name')) {
    ...
    fileListTbody.addEventListener('change', (e) => {
        if (e.target.classList.contains('file-checkbox')) {
            updateSelectionState(e.target.dataset.path); 
        }
    });
    ...
    // Return cleanup function for observers (memory leak prevention)
    pickerContainer._cleanup = () => {
...
export function createfilePicker() {
    ...
    dialogWrapper.style.zIndex = 'var(--z-index-modal, 1000)';

...
export function clearCurrentCacheByName() {
	...
	// ✅ FIX: Check if the panel is NOT 'none' instead of strictly 'block'.
	// This accounts for the switch to `display: flex` for the tab bar layout.
	if (codePanel.style.display !== 'none' && csgEditor.values[csgEditor.valuesIndex]) {
		currentTitle = csgEditor.values[csgEditor.valuesIndex].title
		project.clearMeshCache(currentTitle)
	} else if (editorCodePanel.style.display !== 'none' && editorCodeEditor.values[editorCodeEditor.valuesIndex]) {
...
// ✅ ADAPTER: Bridges optText's Context system to the multi-page 'values' array API
function adaptOptTextInstance(editor, tabBarId) {
	const tabBar = document.getElementById(tabBarId)
	if (!tabBar) return

	function renderTabs() {
		const contexts = editor.listContexts()
		const activeId = editor.contextId
		tabBar.innerHTML = ''

		contexts.forEach((ctx) => {
				const tab = document.createElement('div')
				tab.className = 'tab' + (ctx.id === activeId ? ' active' : '')
				tab.textContent = ctx.name

				tab.addEventListener('click', () => {
						if (editor.contextId !== ctx.id) {
							editor.contextId = ctx.id
							renderTabs()
							editor.dispatchEvent(new CustomEvent('pagechange'))
						}
					})
				...
				if (contexts.length > 1) {
					...
					closeBtn.addEventListener('click', (e) => {
							e.stopPropagation()
							if (confirm(`Close page "${ctx.name}"?`)) {
								editor.removeContext(ctx.id)
								renderTabs()
								editor.dispatchEvent(new CustomEvent('pagechange'))
							}
						})
		...
		addBtn.addEventListener('click', () => {
				const name = prompt('Page name:', `Page ${contexts.length + 1}`)
				if (name) {
					editor.addContext(name, [''], { switchTo: true })
					renderTabs()
					editor.dispatchEvent(new CustomEvent('pagechange'))
				}
			})
	...
	// Re-render tabs if contexts change internally
	editor.addEventListener('optText:change', (e) => {
			if (e.detail?.type === 'context-switched' || e.detail?.type === 'context-removed') {
				renderTabs()
			}
		})
	...
	// Map 'values' property to contexts
	Object.defineProperty(editor, 'values', {
			get() {
				const contexts = editor.dataManager?.contexts || editor.listContexts();
				return contexts.map(ctx => ({
							title: ctx.name,
							content: Array.isArray(ctx.lines) ? ctx.lines.join('\n') : ''
						}))
			},
			set(newPages) {
				const existing = editor.dataManager?.contexts || editor.listContexts();
				for (let i = existing.length - 1; i > 0; i--) {
					editor.removeContext(existing[i].id)
				}
				if (!newPages || newPages.length === 0) {
					const first = (editor.dataManager?.contexts || editor.listContexts())[0]
					if (first) {
						first.name = 'Default'
						editor.contextId = first.id
						editor.value = ''
					}
					renderTabs()
					return
				...
				const first = (editor.dataManager?.contexts || editor.listContexts())[0]
				if (first) {
					first.name = newPages[0].title
					editor.contextId = first.id
					editor.value = newPages[0].content || ''
				}
				for (let i = 1; i < newPages.length; i++) {
					editor.addContext(newPages[i].title, (newPages[i].content || '').split('\n'), { switchTo: false })
				}
				renderTabs()
			}
		})

	// Map 'valuesIndex' property to active context
	Object.defineProperty(editor, 'valuesIndex', {
			get() {
				const contexts = editor.listContexts()
				return contexts.findIndex(ctx => ctx.id === editor.contextId)
			},
			set(idx) {
				const contexts = editor.listContexts()
				if (idx >= 0 && idx < contexts.length) {
					editor.contextId = contexts[idx].id
					renderTabs()
				}
			}
		})

	// Provide a resize method for the console resizer logic
	editor.resize = () => window.dispatchEvent(new Event('resize'))
}
...
export function initialize(domElements) {
	...
	// ✅ NEW: Apply the optText adapter to bridge the API and render tabs
	adaptOptTextInstance(csgEditor, 'csg-tabs')
	adaptOptTextInstance(editorCodeEditor, 'editor-code-tabs')

	// ✅ UPDATED: Bind cache invalidation to 'oninput' and 'onchange'.
	// optText does not dispatch 'optText:change' on every keystroke, it calls 'oninput'.
	// We must assign these functions directly to the properties to capture typing.

	// --- Mesh Cache Invalidation (CSG Editor) ---
	const invalidateMeshCache = function() {
		const pageData = csgEditor.values[csgEditor.valuesIndex]
		if (pageData && pageData.title) {
			const pageName = pageData.title;
			if (project.meshCache[pageName]) project.meshCache[pageName].updated = false
		}
	}
	csgEditor.oninput = invalidateMeshCache
	csgEditor.onchange = invalidateMeshCache

	// --- Code Cache Invalidation (Editor Code) ---

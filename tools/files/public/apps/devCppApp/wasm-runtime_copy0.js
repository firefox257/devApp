...
export class WasmRuntime {
	...
	_initWasi() {
		this.wasiImports = {
			...
			fd_write: (fd, iovsPtr, iovsLen, nwrittenPtr) => {
				...
				// 🚨 CRITICAL: Access .buffer dynamically inside the closure!
				// If WASM memory grows, .buffer returns a NEW ArrayBuffer.
	...
	async instantiate(wasmBinary) {
		...
		// 🚨 FIX 1: Robustly ensure memory is created if missing
		...
		if (!importObject.env.memory) {
			importObject.env.memory = new WebAssembly.Memory({
				...
				shared: true // 🚨 FIX 2: REQUIRED for SharedArrayBuffer / Worker Pool
			});
		}

		// 🚨 FIX 3: Assign memory EARLY!
		// If the WASM module has a start() function (global constructors) that calls fd_write,
		// this.memory MUST be set before WebAssembly.instantiate() resolves.
		...
		try {
			...
			// Fallback to exports.memory just in case (though it shouldn't exist if imported)

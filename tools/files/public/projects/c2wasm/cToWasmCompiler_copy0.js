 ...
 * 
 * ✅ FIXED: Operator precedence, tokenizer patterns, assignment handling
 */
const WASM_TYPE = {
...
class CTokenizer {
  static tokenize(src) {
    ...
    const tokens = [];
    // ✅ FIXED: Multi-char operators MUST come before single-char operators
    const patterns = [
      /^[a-zA-Z_]\w*/,                                    // identifiers
      /^\d+\.\d*([eE][+-]?\d+)?|^\.\d+([eE][+-]?\d+)?|^\d+([eE][+-]?\d+)?/, // ✅ FIXED: numbers including .5
      /^'.'/, /^"[^"]*"/,
      /^==|^!=|^<=|^>=|^&&|^\|\||^[+\-*/%&|^=!<>~]/,    // ✅ FIXED: multi-char ops first!
      /^[(){}[\],;?:]/,
      /^\s+/
    ];
    let i = 0;
...
class CParser {
  constructor(tokens, compiler) {
    ...
    this.continueTarget = [];
    this.lastType = 'i32';
    this.pendingAssignment = null; // ✅ For tracking LHS of assignments
  }
  ...
  parsePrimary() {
    ...
    // ✅ Handle assignment LHS tracking
    if (/^[a-zA-Z_]\w*$/.test(t) && this.tokens[this.pos + 1] === '=') {
      this.pendingAssignment = this.consume(); // store identifier name
      return []; // assignment handled in parseExpression
    }
  ...
  // ✅ FIXED: RHS now correctly calls parseMultiplicative() for proper precedence
  parseAdditive() {
    let ops = this.parseMultiplicative();
    while (['+', '-'].includes(this.peek())) {
      const op = this.consume();
      const rhs = this.parseMultiplicative();  // ✅ WAS: parseAdditive() - BUG!
  ...
  // ✅ FIXED: Proper assignment handling with LHS tracking
  parseExpression() {
  ...
  parseFunction(name, params, retType) {
    ...
    this.expect('}');
    return { params, ret: C_TYPE_MAP[retType], body: [...bodyOps, OP_CODES.end] };
  }
}
...
class CWasmBuilder {
  ...
  addFunction(name, params, retType, bodyOps, isPublic) {
    this.funcs.push({ name, params, retType, body: bodyOps, isPublic, idx: this.funcs.length });
  }
  ...
  generate() {
    ...
    // Code Section: Must include local declaration count
    const code = this.funcs.map(f => {
      const bodyWithLocals = [0x00, ...f.body]; // 0x00 = zero local declarations
    ...
    const sections = [
      { id: 1, data: [types.length, ...types.flat()] },        // Type section
      { id: 3, data: [this.funcs.length, ...funcIdxs] },        // Function section
      { id: 5, data: [1, 0, 1] },                               // Memory section (1 memory, min=1)
      { id: 6, data: [0] },                                     // Global section (empty)
      { id: 7, data: exportsData },                             // Export section
      { id: 10, data: [this.funcs.length, ...code] },           // Code section
      { id: 11, data: [1, 0, 0x41, 0x00, 0x0B, ...uleb(this.data.length), ...this.data] } // Data section
    ];

    let wasm = [0x00, 0x61, 0x73, 0x6d, 0x01, 0x00, 0x00, 0x00]; // magic + version
    ...
    while (parser.pos < parser.tokens.length) {
      if (this._isType(parser.tokens[parser.pos])) {
        if (parser.tokens[parser.pos + 2] === '(') {
          // Reset parser state for new function
          parser.locals = [];
          parser.localIdx = 0;
          ...
          while (parser.peek() !== ')') {
            ...
            if (parser.isType(parser.peek())) {
              ...
              parser.consume(); // skip unknown
            }
          }
          parser.expect(')'); // consume ')'
          
          const func = parser.parseFunction(name, params, retType);
          this.builder.addFunction(name, params, retType, func.body, name === 'main');
        } else {
          parser.consume(); // skip non-function type usage
        }
      } else {

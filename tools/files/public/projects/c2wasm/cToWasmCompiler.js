 ...
 * 
 * ✅ FIXED: Context-aware block typing for proper WASM validation
 */
const WASM_TYPE = {
...
const OP_CODES = {
  ...
  nop: 0x01, end: 0x0B, f32_const: 0x43, unreachable: 0x00
};
...
class CParser {
  constructor(tokens, compiler) {
    ...
    // ✅ NEW: Track expected return type for value-producing contexts
  ...
  // ✅ NEW: Helper to check if ops end with terminal control flow
  _isTerminal(ops) {
    if (!ops || ops.length === 0) return false;
    const last = ops[ops.length - 1];
    return last === OP_CODES.return || last === OP_CODES.br;
  }

  // ✅ NEW: Helper to extract return value from terminal ops
  _extractReturnValue(ops) {
    if (!ops || ops.length < 2) return null;
    // Look for pattern: [...expr, OP_CODES.return]
    if (ops[ops.length - 1] === OP_CODES.return) {
  ...
  parseStatement() {
    ...
    if (t === 'if') {
      ...
      // ✅ BETTER FIX: Context-aware block typing
      // If we expect a return value AND both branches are terminal returns,
      // generate a value-producing if-block instead of control-flow returns
      if (this.expectedReturnType !== WASM_TYPE.void && 
          this.expectedReturnType !== null) {
        const bodyVal = this._extractReturnValue(body);
        const elseVal = this._extractReturnValue(elseBody);
        
        if (bodyVal !== null && elseVal !== null) {
          // Generate: if (cond) (result: type) body_val else else_val end
          return [
            ...cond,
            OP_CODES.if_,
            this.expectedReturnType,  // ✅ Use expected type as block type
            ...bodyVal,
            OP_CODES.else_,
            ...elseVal,
            OP_CODES.end
      ...
      // Fallback: control-flow style with unreachable for validation
      const ifCode = [...cond, OP_CODES.if_, WASM_TYPE.void, ...body, OP_CODES.else_, ...elseBody, OP_CODES.end];
      if (this._isTerminal(body) && this._isTerminal(elseBody)) {
        return [...ifCode, OP_CODES.unreachable];
      }
      return ifCode;
    }
    if (t === 'while') {
  ...
  parseFunction(name, params, retType) {
    ...
    // ✅ NEW: Set expected return type for this function's body
    const prevExpected = this.expectedReturnType;
    this.expectedReturnType = C_TYPE_MAP[retType];
    ...
    // ✅ NEW: Restore previous context
    this.expectedReturnType = prevExpected;
    
    // If function returns non-void and body doesn't end with terminal control flow,
    // ensure we emit a default return (0 for i32, 0.0 for floats)
    const returnType = C_TYPE_MAP[retType];
    if (returnType !== WASM_TYPE.void && returnType !== WASM_TYPE.void) {
      if (!this._isTerminal(bodyOps)) {
        const defaultVal = returnType === WASM_TYPE.i32 ? [OP_CODES.i32_const, 0x00] :
                          returnType === WASM_TYPE.f32 ? [OP_CODES.f32_const, 0x00, 0x00, 0x00, 0x00] :
                          [OP_CODES.i32_const, 0x00];
        bodyOps.push(...defaultVal, OP_CODES.return);
      }
    }
...
class CWasmBuilder {
  ...
  generate() {
    ...
    // Code Section: Dynamic local declarations per function
    ...
    while (parser.pos < parser.tokens.length) {
      if (this._isType(parser.tokens[parser.pos])) {
        if (parser.tokens[parser.pos + 2] === '(') {
          ...
          parser.expectedReturnType = null; // ✅ Reset for each function

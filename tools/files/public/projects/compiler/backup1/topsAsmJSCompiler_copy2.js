...
class TopsAsmJSCompiler {
  constructor(options = {}) {
    ...
    // ✅ FIX #3: Add strictFloatCast option for runtime NaN/Infinity checks
    this.options = { 
      target: 'asmjs',
      strictFloatCast: false,  // Enable strict float→int cast validation
      ...options 
    };
  }
  ...
  tokenize(source) {
    ...
    while (i < source.length) {
      ...
      if (/[0-9]/.test(ch) || (ch === '-' && /[0-9]/.test(source[i+1]))) {
        ...
        if (isFloat) {
          // ✅ FIX #1: parseFloat(num) already handles the sign in num (e.g., "-45.7")
          // No need to apply isNegative again — that caused double-negation!
          tokens.push({ 
            type: 'FLOAT_LITERAL', 
            value: parseFloat(num),  // ✅ FIXED: removed conditional negation
            raw: rawNum
          });
        } else {
          tokens.push({ type: 'INT_LITERAL', value: rawNum, raw: rawNum });
  ...
  // ✅ FIX #3: Helper to check if type is an integer type
  _isIntType(type) {
    return ['int1','int2','int4','int8','uint1','uint2','uint4','uint8','char','bool'].includes(type);
  }
  
  // ✅ FIX #3: Helper to generate strict int cast with NaN/Infinity check
  _wrapStrictIntCast(expr, targetType) {
    const clamp = (type) => {
      ...
      return '(x)|0'; // int4, int8, etc.
    };
    
    // Generate asm.js validation: check for NaN/Infinity before truncation
    // x!==x checks for NaN, x===1/0 and x===-1/0 check for Infinity
  ...
  wrap(expr, type, isPtr) {
    ...
    // ✅ FIX #2: char type now properly truncates to 8-bit unsigned
    if (type === 'char') return `((${expr})&0xFF)>>>0`;
    
    // ✅ FIX #3: Strict float→int validation (optional, opt-in via options)

...
class TopsTokenizer {
	...
	parseStatement() {
		...
		if (lowerT === 'iswitch') {
			...
			// ✅ FIX: Case blocks NO LONGER have implicit break - enables fall-through
			for (const { value, body } of caseBlocks) {
				genCode.push(
					...
					0x04, 0x40,
					...body,
					// REMOVED: 0x0c, 0x01 (br 1) - now allows fall-through
					0x0b
				);
			}
			// Default block still needs break to exit switch
			if (hasDefault) {
				genCode.push(
					0x41, ...this.sleb(1),  // i32.const 1 (always true)
					0x04, 0x40,             // if void
					...defaultCode,
					0x0c, 0x01,             // br 1 to exit switch block (only default breaks)
					0x0b                    // end if
				);
			}

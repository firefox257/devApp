...
// ✅ FIXED: Negative float literal parsing (no double-negation)
// ✅ FIXED: char type cast properly truncates to 8-bit unsigned
// ✅ FIXED: bool type cast converts non-zero → 1, zero → 0
// ✅ FIXED: void type validation - void cannot be used for variables, params, or casts
// ✅ FIXED: @ token consistency - lexer uses '@' type, parser matches '@'
// ✅ FIXED: AddressOf lvalue support - @ptr = value now works for pointer assignment
// ✅ FIXED: 64-bit local variable initialization - registers sync with heap on init
// ✅ FIXED: 64-bit identifier access - use registers for simple reads, heap for address-taken
// ✅ FIXED: float8 unaligned store - use HEAPF64 directly, not byte extraction
// ✅ FIXED: Float coercion - avoid double +/fround on float stores
// ✅ ADDED: Optional strictFloatCast mode for NaN/Infinity validation on float→int casts
// ✅ ADDED: Byte-addressable heap with unaligned load/store support for all types
// ✅ FIXED: Signed single-byte types (int1) use HEAP8 for correct sign extension on >>
...
class TopsAsmJSCompiler {
  ...
  parseFunction() {
    ...
    if (!this.match(')')) {
      do {
        ...
        if (type === 'void') {
          throw new Error(`Parameter cannot have type 'void' in function '${name}'`);
        }
        const isPtr = !!this.match('@');
  ...
  parseStatement() {
    ...
    if (this.isTypeStart()) {
      const type = this.parseType();
      if (type === 'void') {
        throw new Error(`Cannot declare variable of type 'void'`);
      }
      const isPtr = !!this.match('@');
      const name = this.expect('IDENT').value;
      let init = null;
  ...
  validate(ast) {
    ...
    for (const func of ast.body) {
      ...
      const localTypes = new Map(func.params.map(p => [p.name, { type: p.type, isPointer: p.isPointer }]));
      this._validateNode(func.body, scopeStack, func.name, funcSigs, errors, localTypes);
    }
  ...
  _validateNode(node, scopeStack, funcName, funcSigs, errors, localTypes) {
    ...
    if (node.type === 'VarDecl') {
      if (node.varType === 'void') {
        errors.push(`Cannot declare variable '${node.name}' of type 'void' in function '${funcName}'`);
      }
      currentScope.add(node.name);
      localTypes.set(node.name, { type: node.varType, isPointer: node.isPointer });
      if (node.init) this._validateNode(node.init, scopeStack, funcName, funcSigs, errors, localTypes);
      return;
    }
    
    if (node.type === 'Assignment') {
      if (node.left?.type === 'AddressOf') {
        const targetName = node.left.name;
        const targetInfo = localTypes?.get(targetName);
        if (!targetInfo?.isPointer) {
          errors.push(`Cannot assign to '@${targetName}': '${targetName}' is not a pointer variable in function '${funcName}'`);
        }
      }
    ...
    if (node.type === 'Cast') {
      if (node.targetType === 'void') {
        errors.push(`Cannot cast to type 'void' in function '${funcName}'`);
      }
      const sourceType = this._getExprType(node.expression, localTypes, funcSigs);
      if (sourceType === 'void') {
        errors.push(`Cannot cast from type 'void' to '${node.targetType}' in function '${funcName}'`);
      }
    }
    
    if (node.type === 'Block') {
      scopeStack.push(new Set());
      const newLocalTypes = new Map(localTypes);
      for (const stmt of node.statements) {
        this._validateNode(stmt, scopeStack, funcName, funcSigs, errors, newLocalTypes);
      }
      scopeStack.pop();
      return;
    }
    ...
    if (node.type === 'If') {
      this._validateNode(node.test, scopeStack, funcName, funcSigs, errors, localTypes);
      this._validateNode(node.consequent, scopeStack, funcName, funcSigs, errors, localTypes);
      if (node.alternate) this._validateNode(node.alternate, scopeStack, funcName, funcSigs, errors, localTypes);
      return;
    }
    
    if (node.type === 'Loop') {
      this._validateNode(node.body, scopeStack, funcName, funcSigs, errors, localTypes);
      return;
    }
    
    if (node.type === 'Switch') {
      this._validateNode(node.discriminant, scopeStack, funcName, funcSigs, errors, localTypes);
      scopeStack.push(new Set());
      const switchLocalTypes = new Map(localTypes);
      for (const c of node.cases) {
        for (const stmt of c.body) {
          this._validateNode(stmt, scopeStack, funcName, funcSigs, errors, switchLocalTypes);
        }
      }
      scopeStack.pop();
      return;
    ...
    for (const key of Object.keys(node)) {
      ...
      if (Array.isArray(value)) {
        for (const item of value) this._validateNode(item, scopeStack, funcName, funcSigs, errors, localTypes);
      } else if (value && typeof value === 'object' && value.type !== 'FunctionDecl') {
        this._validateNode(value, scopeStack, funcName, funcSigs, errors, localTypes);
      }
    }
  }
  
  _getExprType(expr, localTypes, funcSigs) {
    if (!expr) return null;
    switch (expr.type) {
      case 'Literal': return expr.dataType || 'int4';
      case 'Identifier': {
        if (localTypes?.has(expr.name)) {
          const info = localTypes.get(expr.name);
          return info?.isPointer ? 'int4' : info?.type;
        }
        return 'int4';
      }
      case 'Call': {
        const sig = funcSigs?.get(expr.callee);
        return sig?.returnType || 'int4';
      }
      case 'Cast': return expr.targetType;
      case 'Binary': return expr.resultType || 'int4';
      case 'Unary': return expr.resultType || 'int4';
      case 'SizeOf': return 'int4';
      case 'StringLiteral': return 'int4';
      case 'AddressOf': return 'int4';
      case 'Index': return 'int4';
      default: return 'int4';
    }
  }
  ...
  // ========== BYTE-ADDRESSABLE HEAP HELPERS ==========
  
  allocateVariable(type, isPointer, currentByteOffset) {
    const size = this.sizeOf(type, isPointer);
    const align = this.options.heapAlignment || 1;
    const alignedOffset = align > 1 
      ? Math.ceil(currentByteOffset / align) * align 
      : currentByteOffset;
    return { offset: alignedOffset, nextOffset: alignedOffset + size };
  }

  generateLoad(byteOffsetExpr, type, isPointer = false) {
    if (isPointer) type = 'int4';
    
    const size = this.sizeOf(type, false);
    const view = this.getHeapView(type);
    const shift = this.getShiftAmount(type);
    
    if (size <= 4 && size === (1 << shift) && this.options.heapAlignment <= size) {
      return `${view}[(${byteOffsetExpr})>>${shift}]`;
    }
    
    if (type === 'float8') {
      return `+HEAPF64[(${byteOffsetExpr})>>3]`;
    }
    
    if (type === 'int8' || type === 'uint8') {
      const lo = `HEAPU32[(${byteOffsetExpr})>>2]`;
      const hi = `HEAPU32[(((${byteOffsetExpr})|0)+4)>>2]`;
      return `[${hi}|0, ${lo}|0]`;
    }
    ...
    if (size === 2) {
      ...
      if (littleEndian) {
        return `((HEAPU8[${byteOffsetExpr}|0]|0) | ((HEAPU8[((${byteOffsetExpr})|0)+1|0]|0)<<8))|0`;
      } else {
        return `((HEAPU8[((${byteOffsetExpr})|0)+1|0]|0) | ((HEAPU8[${byteOffsetExpr}|0]|0)<<8))|0`;
      }
    }
    
    return this._generateUnalignedLoad(byteOffsetExpr, type, size);
  }

  _generateUnalignedLoad(byteOffset, type, size) {
    if (this._isFloat64(type)) {
      return `+(function(p){var b=new stdlib.DataView(heap,p);return b.getFloat64(0,true)})(${byteOffset})`;
    }
    ...
    for (let i = 0; i < size; i++) {
      const shift = i * 8;
      parts.push(`(HEAPU8[((${byteOffset})|0)+${i}|0]|0)${shift > 0 ? `<<${shift}` : ''}`);
    }
    const expr = parts.join('|');
    
    if (type?.startsWith('int') && !type.startsWith('uint')) {
      const bits = size * 8;
      return `((${expr})<<${32-bits})>>${32-bits}`;
    }
    const mask = type?.startsWith('uint') ? `>>>0` : `|0`;
    return `(${expr})${mask}`;
  }

  generateStore(byteOffsetExpr, valueExpr, type, isPointer = false) {
    if (isPointer) type = 'int4';
    ...
    if (size <= 4 && size === (1 << shift) && this.options.heapAlignment <= size) {
      // ✅ FIX: float types already coerced by wrap(), don't double-coerce
      if (type?.startsWith('float')) {
        return `${view}[(${byteOffsetExpr})>>${shift}] = ${valueExpr}`;
      }
      const coerced = this._coerceForType(valueExpr, type);
      return `${view}[(${byteOffsetExpr})>>${shift}] = ${coerced}`;
    }
    
    // ✅ FIX: float8 special case - no re-coercion
    if (type === 'float8') {
      return `HEAPF64[(${byteOffsetExpr})>>3] = ${valueExpr}`;
    }
    
    if (type === 'int8' || type === 'uint8') {
      const [hi, lo] = typeof valueExpr === 'string' && valueExpr.startsWith('[')
        ? valueExpr.match(/^\[\s*([^,]+)\s*,\s*([^,\]]+)\s*\]$/)?.slice(1).map(x => x.trim())
        : [`${valueExpr}_hi`, `${valueExpr}_lo`];
      return [
        `HEAPU32[(${byteOffsetExpr})>>2] = ${lo}|0`,
        `HEAPU32[(((${byteOffsetExpr})|0)+4)>>2] = ${hi}|0`
      ].join(';');
    }
    
    return this._generateUnalignedStore(byteOffsetExpr, valueExpr, type, size);
  }

  _generateUnalignedStore(byteOffset, value, type, size) {
    const stmts = [];
    const coerced = this._coerceForType(value, type);
    
    if (size === 1) {
      const view = this.getHeapView(type);
      return `${view}[(${byteOffset})|0] = ${coerced}|0`;
    }
    
    for (let i = 0; i < size; i++) {
      const byteVal = `((${coerced})>>${i*8})&0xFF`;
      stmts.push(`HEAPU8[((${byteOffset})|0)+${i}|0] = ${byteVal}|0`);
    }
    return stmts.join(';');
  }

  _coerceForType(expr, type) {
    if (!expr) expr = '0';
    if (type === 'bool') return `((${expr})|0)!=0?1:0`;
    if (type === 'char') return `((${expr})&0xFF)>>>0`;
    if (type?.startsWith('uint')) {
      if (type === 'uint1') return `((${expr})&0xFF)>>>0`;
      if (type === 'uint2') return `((${expr})&0xFFFF)>>>0`;
      return `(${expr})>>>0`;
    }
    if (type?.startsWith('int')) return `(${expr})|0`;
    if (type?.startsWith('float')) return this._isFloat64(type) ? `+${expr}` : `fround(${expr})`;
    return expr;
  }
  ...
  _isIntType(type) {
    return ['int1','int2','int4','int8','uint1','uint2','uint4','uint8','char','bool'].includes(type);
  }
  
  _wrapStrictIntCast(expr, targetType) {
    const clamp = (type) => {
      if (type === 'int1') return '((x<<24)>>24)';
      if (type === 'int2') return '((x<<16)>>16)';
      if (type === 'uint1') return '((x)&0xFF)>>>0';
      if (type === 'uint2') return '((x)&0xFFFF)>>>0';
      if (type?.startsWith('uint')) return '(x)>>>0';
      return '(x)|0';
    };
    return `(function(x){if(x!==x||x===1/0||x===-1/0){throw new Error("Out of bounds Trunc operation")}return ${clamp(targetType)}})(${expr})`;
  }
  ...
  compile(ast) {
    ...
    if (ast.strings.length > 0) {
      lines.push('  // String literals (byte offsets)');
      for (let i = 0; i < ast.strings.length; i++) {
  ...
  compileFunction(func, lines, stringOffsets, funcSigs) {
    ...
    const localByteOffsets = new Map();
    
    for (const param of params) {
      const size = this.sizeOf(param.type, param.isPointer);
      nextByteOffset += size;
    }
    
    for (const [lname, linfo] of locals) {
      const alloc = this.allocateVariable(linfo.type, linfo.isPointer, nextByteOffset);
      localByteOffsets.set(lname, { ...linfo, byteOffset: alloc.offset });
      nextByteOffset = alloc.nextOffset;
    }
    ...
    for (const stmt of body.statements) {
      const code = this.compileStmt(stmt, stringOffsets, funcSigs, returnType, params, localByteOffsets);
  ...
  compileStmt(stmt, strOff, sigs, funcReturnType = 'int4', funcParams = [], localVarTypes = new Map()) {
    switch (stmt.type) {
      case 'VarDecl': {
        ...
        const varInfo = localVarTypes?.get(name);
        const byteOffset = varInfo?.byteOffset;
        ...
        if (byteOffset !== undefined && init) {
          const store = this.generateStore(`${byteOffset}`, val, varType, isPointer);
          
          if ((varType === 'int8' || varType === 'uint8') && !isPointer && typeof val === 'string' && val.startsWith('[')) {
            const match = val.match(/^\[\s*([^,]+)\s*,\s*([^,\]]+)\s*\]$/);
            if (match) {
              const [, hiExpr, loExpr] = match;
              const regAssign = `${name}_hi = ${hiExpr}|0; ${name}_lo = ${loExpr}|0;`;
              if (Array.isArray(store)) {
                return [...store.split(';').filter(s => s.trim()).map(s => s + ';'), regAssign];
              }
              return store + ';' + regAssign;
            }
          }
          
          if (Array.isArray(store)) {
            return store.split(';').filter(s => s.trim()).map(s => s + ';');
          }
          return store + ';';
        }
  ...
  compileExpr(expr, strOff, sigs, expectedType = 'int4', funcParams = [], localVarTypes = new Map()) {
    ...
    switch (expr.type) {
      ...
      case 'Identifier': {
        const varInfo = localVarTypes?.get(expr.name);
        
        if ((expectedType === 'int8' || expectedType === 'uint8') && expr.name !== '$result') {
          return `[${expr.name}_hi|0, ${expr.name}_lo|0]`;
        }
        
        if (varInfo?.byteOffset !== undefined) {
          return this.generateLoad(`${varInfo.byteOffset}`, varInfo.type, varInfo.isPointer);
        }
        return expr.name;
      }
      ...
      case 'Index': {
        ...
        const elemSize = this.sizeOf(elemType || 'int4', false);
        
        const byteAddr = elemSize === 1 
          ? `((${ptr}|0) + (${idx}|0))|0`
          : `((${ptr}|0) + (imul(${idx}|0, ${elemSize})|0))|0`;
        
        return this.generateLoad(byteAddr, elemType, false);
      }
      ...
      case 'Unary': {
        ...
        if (op === '-') {
          ...
          if (expectedType === 'int8' || expectedType === 'uint8') {
            const [hi, lo] = typeof arg === 'string' && arg.startsWith('[')
              ? arg.match(/^\[\s*([^,]+)\s*,\s*([^,\]]+)\s*\]$/)?.slice(1).map(x => x.trim())
              : [`${arg}_hi`, `${arg}_lo`];
      ...
      case 'Assignment': {
        if (expr.left.type === 'AddressOf') {
          const ptrName = expr.left.name;
          const rhs = this.compileExpr(expr.right, strOff, sigs, 'int4', funcParams, localVarTypes);
          return `${ptrName} = (${rhs})|0;`;
        }
        ...
        if (expr.left.type === 'Index') {
          ...
          const store = this.generateStore(byteAddr, val, elemType, false);
          return Array.isArray(store) ? store.split(';').filter(s => s.trim()).map(s => s + ';') : store + ';';
        }
        ...
        if (expr.left.type === 'Identifier') {
          const varInfo = localVarTypes?.get(expr.left.name);
          if (varInfo?.byteOffset !== undefined) {
            const rhs = this.compileExpr(expr.right, strOff, sigs, expectedType, funcParams, localVarTypes);
            const store = this.generateStore(`${varInfo.byteOffset}`, rhs, varInfo.type, varInfo.isPointer);
            return Array.isArray(store) ? store.split(';').filter(s => s.trim()).map(s => s + ';') : store + ';';
          }
        }
      ...
      case 'Cast': {
        if (expr.expression?.dataType === 'void') {
          throw new Error(`Cannot cast from type 'void' to '${expr.targetType}'`);
        }
  ...
  wrap(expr, type, isPtr) {
    if (type === 'void') {
      throw new Error(`INTERNAL: void type has no runtime representation`);
    }
    ...
    if (type === 'char') return `((${expr})&0xFF)>>>0`;
    if (type === 'bool') return `(((${expr})|0)!=0)|0`;
    
    if (this.options.strictFloatCast && this._isIntType(type)) {
      return this._wrapStrictIntCast(expr, type);
    }

...
var Descriptor = class _Descriptor {
	...
	// Synchronous WebSocket request wrapper
	_req(cmd, params = {}) {
		...
		const handler = (e) => {
			...
			if (res.id === id) {
				...
				result = res.result; error = res.error; done = true;
			}
		};
		this.ws.addEventListener('message', handler);
		this.ws.send(JSON.stringify({ id, cmd, fd: this.fd, ...params }));

		const start = performance.now();
		while (!done) {
			if (performance.now() - start > 5000) throw 'io';
			// Yield to event loop without breaking sync contract
			if (typeof SharedArrayBuffer !== 'undefined') {
				const buf = new SharedArrayBuffer(4);
				Atomics.wait(new Int32Array(buf), 0, 0, 1);
			}
		}
	...
	getType() { return this._req('get-type').type; }
	getFlags() { return {}; }

	metadataHash() {
		const r = this._req('metadata-hash');
		return { upper: Number(r.hash.upper), lower: Number(r.hash.lower) };
	}

	metadataHashAt(_pathFlags, path) {
		const r = this._req('metadata-hash', { path });
		return { upper: Number(r.hash.upper), lower: Number(r.hash.lower) };
	}

	stat() {
		const r = this._req('stat');
		return {
			type: r.type, linkCount: Number(r.nlink), size: Number(r.size),
			dataAccessTimestamp: null, dataModificationTimestamp: null, statusChangeTimestamp: null
		};
	}

	statAt(_pathFlags, path) {
		const r = this._req('stat', { path });
		return {
			type: r.type, linkCount: Number(r.nlink), size: Number(r.size),
			dataAccessTimestamp: null, dataModificationTimestamp: null, statusChangeTimestamp: null
		};
	...
	openAt(_pathFlags, path, openFlags, descriptorFlags) {
		const r = this._req('open-at', { path, oflags: openFlags, flags: descriptorFlags });
		return new _Descriptor(r.fd, this.ws);
	}

	read(length, offset) {
		const r = this._req('read', { length: Number(length), offset: Number(offset) });
		const data = Uint8Array.from(atob(r.data[0] || ''), c => c.charCodeAt(0));
		return [data, r.bytesRead < Number(length)];
	}

	readViaStream(offset) { throw 'unsupported'; }

	write(buffer, offset) {
		const r = this._req('write', {
				data: btoa(String.fromCharCode(...buffer)),
				offset: Number(offset)
			});
		return Number(r.bytesWritten);
	}

	writeViaStream(offset) { throw 'unsupported'; }

	setSize(size) { this._req('set-size', { size: Number(size) }); }

	readDirectory() { return new DirectoryEntryStream(this.fd, this.ws); }

	createDirectoryAt(path) { this._req('create-directory-at', { path }); }
	unlinkFileAt(path) { this._req('unlink-file-at', { path }); }
	removeDirectoryAt(path) { this._req('remove-directory-at', { path }); }
	readlinkAt(path) { return this._req('readlink', { path }).target; }

	renameAt(oldPath, newDescriptor, newPath) {
		this._req('rename', { path: oldPath, newpath: newPath, newDirFd: newDescriptor.fd });
	}

	setTimes(atime, mtime) { this._req('set-times', { times: { atim: atime, mtim: mtime } }); }
	symlinkAt(target, linkpath) { this._req('symlink', { target, newpath: linkpath }); }
	linkAt(oldFlags, oldPath, newDescriptor, newPath) {
		this._req('link', { path: oldPath, newpath: newPath, newDirFd: newDescriptor.fd });
	}
	appendViaStream() { throw 'unsupported'; }
};
var DirectoryEntryStream = class {
	...
	_loadSync() {
		...
		const handler = (e) => {
			...
			if (res.id === id) {
				this.ws.removeEventListener('message', handler);
				result = res.result; done = true;
			}
		};
		this.ws.addEventListener('message', handler);
		this.ws.send(JSON.stringify({ id, cmd: 'readdir', fd: this.fd }));
...
var Environment = class {
	vars = {}; args = []; root = null; ws = null; wsReady = false;
	...
	constructor() {
		...
		// Connect to fs-proxy.ws.js server
		//`ws://${window.location.host}${this.url}`;
		//this.ws = new WebSocket("ws://local/wasi/fs-proxy.ws.js");
		var fsurl = `ws://${window.location.host}/wasi/fs-proxy.ws.js`;
		console.log(fsurl);
		this.ws = new WebSocket(fsurl);

		this.ws.addEventListener('open', () => { this.wsReady = true; });

		// Sync wait for connection
		const start = performance.now();
		while (!this.wsReady && performance.now() - start < 5000) {
			if (typeof SharedArrayBuffer !== 'undefined') {
				const buf = new SharedArrayBuffer(4);
				Atomics.wait(new Int32Array(buf), 0, 0, 1);
			}
		}
		if (!this.wsReady) throw new Error("Failed to connect to FS proxy");

		const $this = this;
		this.exports = {
			monotonicClock: { now: monotonicNow },
			wallClock: { now: wallClockNow },
			random: { getRandomBytes(length) { return $this.prng.getBytes(Number(length)); } },
			io: { Error: IoError, InputStream, OutputStream },
			cli: {
				exit(status) { throw new Exit(status.tag === "ok " ? 0 : 1); },
				getEnvironment() { return $this.vars; },
				getArguments() { return $this.args; },
				getStdin() { return $this.standardInputStream; },
				getStdout() { return $this.standardOutputStream; },
				getStderr() { return $this.standardErrorStream; },
			...
			fs: {
				...
				filesystemErrorCode() {},
				getDirectories() {
					// Return preopen FD 3 provided by the proxy server
					return [[new Descriptor(3, $this.ws), "/ "]];
				}
			}
		};
	}

	get stdin() { return this.standardInputStream.callback; }
	set stdin(cb) { this.standardInputStream.callback = cb; }
	get stdout() { return this.standardOutputStream.callback; }
	set stdout(cb) { this.standardOutputStream.callback = cb; }
	get stderr() { return this.standardErrorStream.callback; }
	set stderr(cb) { this.standardErrorStream.callback = cb; }
};

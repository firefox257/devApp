...
function createOptTextInstance(originalElement = null, initialData = null) {
	...
	function _render(timestamp) {
		...
		// ✅ Keep the toolbar button perfectly in sync with the selection state
	...
	// ✅ NEW: Sync the toggle-select button with selection state
	...
	function _setupEvents() {
		...
		dropdown.addEventListener('click', e => {
			...
			switch (it.dataset.action) {
				// ✅ REMOVED: 'start-select' and 'clear-selection' cases
		...
		container.addEventListener('click', e => {
			...
			if (b.dataset.action === 'toggle-select') {
				if (selection.active) {
					_clearSelection(); // Turns button back to blue
				} else {
					...
					needsRender = true; // Turns button red
				}
			}
		});

...
.opt-text-toolbar-btn:hover:not(:disabled) {
background: var(--ot-hover-toolbar, rgba(0,0,0,0.06));
}
.opt-text-toolbar-btn:active:not(:disabled) {
...
.opt-text-toolbar-btn:hover:not(:disabled) svg {
opacity: 1;
}
.opt-text-toolbar-btn[aria-expanded="true"] {
background: var(--ot-active-toolbar, rgba(26, 115, 232, 0.12));
}
.opt-text-toolbar-btn[aria-expanded="true"] svg {
opacity: 1;
stroke: #1a73e8;
}
/* ===== DROPDOWN MENU ===== */
...
.opt-text-dropdown-item:hover {
background: var(--ot-hover-dropdown, #f5f5f5);
}
.opt-text-dropdown-item:active {
background: var(--ot-active-dropdown, #e8e8e8);
}
.opt-text-dropdown-item.danger {
color: var(--ot-danger, #dc3545);
}
.opt-text-dropdown-item.danger:hover {
background: var(--ot-hover-danger, #fff0f0);
}
...
.opt-text-scrollbar.visible {
opacity: 1;
pointer-events: auto;
}
.opt-text-scrollbar.vertical {
width: 4px;
right: 3px;
top: 31px;
bottom: 3px;
}
.opt-text-scrollbar.horizontal {
height: 4px;
left: 3px;
right: 3px;
bottom: 3px;
...
.opt-text-scrollbar.vertical .opt-text-scrollbar-thumb {
width: 100%;
left: 0;
}
.opt-text-scrollbar.horizontal .opt-text-scrollbar-thumb {
height: 100%;
top: 0;
}
/* ===== HIDDEN INPUT ===== */
...
.opt-text-cursor-preview.visible {
opacity: 1;
}
/* ===== LOADING OVERLAY ===== */
.opt-text-loading {
...
.opt-text-loading-text {
color: var(--ot-text-loading, #000);
font-size: 14px;
margin-bottom: 8px;
}
.opt-text-loading-progress {
color: var(--ot-text-loading-dim, #666);
font-size: 12px;
}
...
.opt-text-modal-overlay.visible .opt-text-modal-dialog {
transform: translateY(0);
}
...
.opt-text-modal-stat-value {
font-weight: 700;
color: var(--ot-modal-stat-value, #1a1a1a);
font-size: 14px;
}
.opt-text-modal-stat-value.danger {
color: var(--ot-danger, #dc3545);
}
.opt-text-modal-stat-label {
color: var(--ot-modal-stat-label, #888);
font-size: 11px;
margin-top: 2px;
}
.opt-text-modal-buttons {
display: flex;
gap: 10px;
}
.opt-text-modal-btn {
...
.opt-text-modal-btn:active {
transform: scale(0.98);
}
.opt-text-modal-btn-cancel {
background: var(--ot-modal-btn-cancel, #f0f0f0);
color: var(--ot-modal-btn-cancel-text, #333);
}
.opt-text-modal-btn-cancel:hover {
background: var(--ot-modal-btn-cancel-hover, #e0e0e0);
}
.opt-text-modal-btn-confirm {
background: var(--ot-danger, #dc3545);
color: #ffffff;
}
.opt-text-modal-btn-confirm:hover {
background: var(--ot-danger-hover, #c82333);
}
/* ===== GOTO LINE INPUT ===== */
.opt-text-goto-input {
width: 100%;
...
.opt-text-goto-input::placeholder {
color: #999;
}
/* ===== TOAST ===== */
.opt-text-toast {
...
@media (hover: none) {
.opt-text-dropdown-item:active {
background: var(--ot-active-dropdown-touch, #d0d0d0);
}
}
`;
...
export function showToast(message, container) {
  ...
  if (!document.getElementById('opt-text-toast-style')) {
    ...
    style.textContent = `@keyframes ot-toast-fade { 0% { opacity: 0; transform: translateX(-50%) translateY(10px); } 15% { opacity: 1; transform: translateX(-50%) translateY(0); } 85% { opacity: 1; transform: translateX(-50%) translateY(0); } 100% { opacity: 0; transform: translateX(-50%) translateY(-10px); } }`;
    document.head.appendChild(style);
  }
...
export function getGotoLineModal(container) {
  ...
  if (!modal) {
    ...
    modal.innerHTML = `<div class="opt-text-modal-dialog" style="max-width:320px;padding:16px">
      <div class="opt-text-modal-handle"></div>
      <div class="opt-text-modal-title">Go to Line</div>
      <div class="opt-text-modal-message" style="margin-bottom:12px">Enter line number (1–<span data-ref="max">0</span>)</div>

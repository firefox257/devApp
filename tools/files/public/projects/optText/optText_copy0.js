...
import { TextDataManager, TextContext, EditCommand } from './optTextData.js'; // ✅ Added EditCommand
...
export function setupOptTextInstance(originalElement = null, initialData = null) {
	...
	if (originalElement && typeof additionManager !== 'undefined' && additionManager.registry) {
		for (const def of additionManager.registry.values()) {
			...
			// ⬇️ ADD THIS EXACT BLOCK RIGHT HERE ⬇️
			if (def.stateConfig) {
				for (const [stateName, config] of Object.entries(def.stateConfig)) {
					if (originalElement.hasAttribute(config.attribute)) {
						...
						// Store with underscore prefix so the addition's init getter can read it
	...
	if (typeof additionManager !== 'undefined') {
		// 1. Inject toolbar buttons FIRST so they exist in the DOM for any init functions
		if (typeof additionManager.injectToolbarButtons === 'function') {
			additionManager.injectToolbarButtons(container, dataManager, container.extensions);
		}
		// 2. Inject dropdown items
		if (typeof additionManager.injectDropdownItems === 'function') {
			additionManager.injectDropdownItems(container, dataManager, container.extensions);
		}
		// 3. THEN run auto-init additions (which may need to query the newly created buttons)
		if (typeof additionManager.injectAutoInitAdditions === 'function') {
	...
	// ✅ GITHUB-LIKE DIFFING ALGORITHM (ONLY FOR ADDITIONS)
	function _createDiffCommand(before, after, reason) {
		...
		// Pre-populate affected lines so undo/redo works perfectly without needing to execute first
	...
	// ✅ NORMAL TYPING: Uses Command Coalescing
	function _insertTextAtCursor(text, recordHistory = true) {
		...
		if (selection.active) {
			...
			if (r) {
				_replaceSelectionWithText(text, r, recordHistory);
				return;
			}
		}
		...
		if (recordHistory && dataManager.current) {
			dataManager.current.history.push(command); // ✅ COALESCING HAPPENS HERE
			_updateUndoRedoButtons();
		}
	...
	function _confirmReplaceAll() {
		...
		// Replace All is an addition-like bulk operation, so we use the diffing algorithm
		const snapshotBefore = _getDocumentSnapshot();
	...
	function _handleKey(e) {
		...
		switch (e.key) {
			...
			if (cursor.col > 0) {
				...
				// ✅ FIX: Set endCol to 0 so the entire current line is preserved and appended
	...
	function _setupEvents() {
		...
		hiddenInput.addEventListener('input', e => {
				...
				_insertTextAtCursor(e.data, true); // ✅ COALESCING ENABLED FOR TYPING
				hiddenInput.value = '';
...
export function createOptText() {
	...
	w.className = 'opt-text-dialog-wrapper';
	w.style.cssText = 'position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);width:80%;height:80%;max-width:600px;max-height:800px;z-index:99999999999;';
	w.appendChild(setupOptTextInstance());
	return w;
}
...
if (typeof document !== 'undefined') {
	const initAutoTags = () => {
		document.querySelectorAll('opttext').forEach(el => {
				...
				if (p) p.replaceChild(setupOptTextInstance(el), el);
			});
	};
	if (document.readyState === 'loading') {
		document.addEventListener('DOMContentLoaded', initAutoTags);
	} else {
		initAutoTags();
	}
}